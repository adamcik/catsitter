...
@register('(?P<spotifyline>.*(spotify:(track|artist|album):[A-Za-z0-9]+|open.spotify.com/(track|artist|album)/[A-Za-z0-9]+).*)')
def handler(spotifyline=None):
    result = []

    for type, id in re.findall('spotify:(track|artist|album):([A-Za-z0-9]+)', spotifyline):
        fetch(type, id, result)
    for type, id in re.findall('open.spotify.com/(track|artist|album)/([A-Za-z0-9]+)', spotifyline):
        fetch(type, id, result)

    return result

def fetch(type, id, result):
    try:
